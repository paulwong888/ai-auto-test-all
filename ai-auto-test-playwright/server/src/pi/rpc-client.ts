...
export class PiRpcClient extends EventEmitter {
  ...
  async promptAndWait(message: string, timeoutMs = 600_000): Promise<void> {
    this.resetSettled();
  ...
  async waitUntilSettled(timeoutMs: number): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onProgress = (event: PiProgressEvent) => {
  ...
  resetSettled(): void {
    ...
    this.streamState.lastStreamError = null;
  ...
  getLastStreamError(): string | null {
    return this.streamState.lastStreamError;
  }
  ...
  private emitProgress(event: PiProgressEvent): void {
