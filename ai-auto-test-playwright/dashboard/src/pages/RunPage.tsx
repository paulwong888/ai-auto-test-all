...
export function RunPage() {
  ...
  const { logs, running, result, resetLive } = useRunWebSocket();
  ...
  const startRun = async (body: Record<string, unknown>) => {
    ...
    resetLive();
    try {
      await fetchJson(`/api/projects/${id}/run`, {
        method: "POST",
  ...
  const runWithPreset = () => {
    if (preset === "custom") {
      void startRun({ preset: "custom", headed, slowmo: slowmo || undefined, specFilter: selectedTc || null });
      return;
    }
